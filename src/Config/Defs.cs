...
public sealed record BalanceDef
{
    ...
    /// <summary>Base XP for the in-run commander level curve — every level is one card
    /// draft, so this is the main knob for how many cards a run hands you.</summary>
    public float RunXpPerLevel { get; init; } = 15f;
    public float RunXpCurve { get; init; } = 1.5f;
...
public sealed record RunCardDef
{
    ...
    /// <summary>Card art as <see cref="Sentinel.Render.Art.Pdtd"/> takes it (e.g.
    /// "tiles/missile"). Boost cards had none, so they fell through to a flat gradient —
    /// which is what "Field Amplifier / Saturation Doctrine / Multi-Launch have no art"
    /// meant. Blank still falls back.</summary>
...
/// <summary>One of PDTD's in-run upgrade cards (data/skillcards.json, generated by
/// tools/gen_skillcards.py). Unlike the old one-card-per-weapon model, a weapon has a
/// whole menu of these — "Power Link", "Extended Reach", "Link Burst" — each with its own
/// text, its own cap, and its own gates.</summary>
public sealed record SkillCardDef
{
    public string Id { get; init; } = "";
    /// <summary>Orbital weapon <c>Kind</c> this belongs to, or "battery" for the planet's
    /// missile battery (which isn't an orbital weapon in Beyond).</summary>
    public string Kind { get; init; } = "";
    public string Weapon { get; init; } = "";
    public string Title { get; init; } = "";
    public string Text { get; init; } = "";
    /// <summary>The "Release a X Sentinel" card — puts the weapon into play at level 1.</summary>
    public bool Unlock { get; init; }
    /// <summary>How many times this card may be taken in one run.</summary>
    public int MaxPicks { get; init; } = 1;
    /// <summary>Weapon level required before this is offered.</summary>
    public int NeedLevel { get; init; }
    /// <summary>Weapon promotions (level-ups) required before this is offered — PDTD's
    /// depSuperStar.</summary>
    public int NeedStar { get; init; }
    public int Weight { get; init; } = 1000;
    /// <summary>True when the card stands in for a PDTD mechanic Beyond doesn't simulate
    /// and carries a damage bonus instead. Informational only.</summary>
    public bool Approx { get; init; }
    public System.Collections.Generic.Dictionary<string, float> Effects { get; init; } = new();
}

public sealed record SkillCardsDb
{
    public System.Collections.Generic.List<SkillCardDef> Cards { get; init; } = new();
}
