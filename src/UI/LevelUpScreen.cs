...
public sealed partial class LevelUpScreen : CanvasLayer
{
    ...
    private void Rebuild()
    {
        ...
        foreach (var card in offer)
        {
            ...
            cat.AddThemeFontSizeOverride("font_size", 20);
            col.AddChild(cat);
            var nm = new Label { Text = card.Name };
            nm.AddThemeFontSizeOverride("font_size", 29);
            col.AddChild(nm);
            var tx = new Label { Text = card.Text, AutowrapMode = TextServer.AutowrapMode.WordSmart, Modulate = new Color(1, 1, 1, 0.72f) };
            tx.AddThemeFontSizeOverride("font_size", 21);
            col.AddChild(tx);
            var btn = new Button { Text = "Take", CustomMinimumSize = new Vector2(0, 87) };
            btn.AddThemeFontSizeOverride("font_size", 31);
            string id = card.Id;
