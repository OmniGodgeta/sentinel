...
{
    None,
    SetHeroTarget,   // Pos = drag destination
    FireVolley,      // Pos = aim point
    CastAbility,     // IntA = slot, Pos = reticle/arc center
    BuildTurret,     // IntA = slot, StrA = turret def id
    RotateTurret,    // IntA = slot, FloatA = angle radians
    SellTurret,      // IntA = slot
    StartWave,
}

/// <summary>
/// One queued player intent. Every input becomes one of these and is applied at
/// the top of a sim tick — nothing in the game reads live input mid-tick, which
/// is what makes 4x tapping safe and the run reproducible.
/// </summary>
