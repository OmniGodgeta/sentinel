...
For *what's left to do* see **`ROADMAP.md`**. For agent ground rules see
**`../CLAUDE.md`**.
...
| §4 Hero level 1–20 with milestone ability-slot unlocks | **Built** — slots 3 → 4 @ hero L8 → 5 @ L20, plus level-card bonuses. | — |
...
| §14 Loadout presets | **Built (v0.11.0)** — 3 slots, Protocols screen. | User: build this early to de-friction the balance pass. |
| §14 hold-to-inspect, Simulate, cloud save | Not built. | Later polish pass — see ROADMAP. |
| Balance | **Untuned, and it's the blocker.** All knobs are in `data/*.json` (`data/survival.json` is the whole spawn loop). Nothing new gets built until the survival loop is proven on cheap hardware at 4×. | User, 2026-09-10. |
